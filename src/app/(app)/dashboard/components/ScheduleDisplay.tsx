...
import type { TimeSlot, User, Booking, MatchPadelLevel, SortOption, Instructor } from '@/types'; // Changed ClassPadelLevel
import { matchPadelLevels } from '@/types'; // Use matchPadelLevels
import TimeSlotCard from './TimeSlotCard';
import InstructorPanel from '@/components/instructor/InstructorPanel';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import { fetchTimeSlots, mockInstructors, fetchInstructors, updateUserFavoriteInstructors, isSlotEffectivelyCompleted, getAllMockUserBookings, mockTimeSlots as importedMockTimeSlots } from '@/lib/mockData';
...
const ScheduleDisplay: React.FC<ScheduleDisplayProps> = ({ currentUser: initialCurrentUser, onBookingSuccess }) => {
  ...
  const [selectedLevel, setSelectedLevel] = useState<MatchPadelLevel | 'all'>("all"); // Changed type
  ...
  const isInstructor = !!mockInstructors.find(inst => inst.id === currentUser.id);
  ...
  const applyFiltersAndSort = useCallback((slotsToFilter: TimeSlot[]): TimeSlot[] => {
    ...
    if (!isInstructor) {
        if (selectedLevel !== 'all') {
            filteredSlots = filteredSlots.filter(slot => slot.level === selectedLevel || slot.level === 'abierto'); // Allow "abierto" to always pass level filter unless specific level is chosen
        }
       ...
       const instructorData = mockInstructors.find(inst => inst.id === currentUser.id);
       if (instructorData) {
