...
import type { Club, PadelCourt, Match, MatchPadelLevel, PadelCategoryForSlot, User, NumericMatchPadelLevel } from '@/types';
import { matchPadelLevels, padelCategoryForSlotOptions, daysOfWeek, numericMatchPadelLevels } from '@/types';
import { Calendar } from "@/components/ui/calendar";
import { CalendarDays, Users2 as CategoryIcon } from "lucide-react"; 
import { format, getDay, addMinutes, startOfDay, setMinutes, setHours } from "date-fns";
...
import { addMatch, getMockStudents, getMockClubs } from '@/lib/mockData';
...
interface OpenMatchFormForAdminProps {
    club: Club;
...
const matchSchema = z.object({
  ...
  clubId: z.string(), // Always required, but will be set programmatically
});
...
const OpenMatchFormForAdmin: React.FC<OpenMatchFormForAdminProps> = ({ club, clubPadelCourts, onMatchOpened }) => {
    ...
    const onSubmit = (values: MatchFormData) => {
        startTransition(async () => {
            ...
            const bookedPlayersData = selectedPlayers.map(userId => {
                ...
                return { userId: userId, name: player?.name || 'Unknown' };
            });
            ...
            if ('error' in result) {
                ...
                form.reset({
                    ...
                    level: 'abierto',
                    category: 'abierta',
                    totalCourtFee: calculateActivityPrice(club, new Date()), // Recalculate for current time
                    clubId: club.id,
                });
                setSelectedPlayers([]);
            }
                    ...
                    <FormField control={form.control} name="level" render={({ field }) => (
                        <FormItem className="flex-1">
                            ...
                            <Select onValueChange={field.onChange} value={String(field.value)}>
                                ...
                                <SelectContent>
                                    <SelectItem value="abierto">Nivel Abierto</SelectItem>
