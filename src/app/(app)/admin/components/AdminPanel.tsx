...
const AdminPanel: React.FC<AdminPanelProps> = ({ adminClub }) => {
    ...
    { value: "createMatchPro", label: "Crear Partida Fija", icon: Trophy, contentDescription: `Configura y abre una nueva partida fija (nivel avanzado) en tu club.`, componentFactory: (props) => (
        ...
        { value: "clubSettings", label: "Ajustes Club", icon: Settings, contentDescription: "Configura las actividades y la visibilidad de las pestañas para los usuarios.", componentFactory: (props) => (
             <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg bg-secondary/30"><div className="space-y-0.5"><Label htmlFor="matchpro-visibility" className="text-base font-medium flex items-center"><Trophy className="mr-2 h-5 w-5 text-slate-500" />Activar Partidas Fijas (Pro)</Label><p className="text-xs text-muted-foreground">Permite crear y mostrar partidas fijas o de nivel avanzado.</p></div><Switch id="matchpro-visibility" checked={props.club.isMatchProEnabled ?? false} onCheckedChange={(checked) => handleToggleActivity('matchPro', checked)} disabled={isUpdatingSettings} aria-label="Activar o desactivar Partidas Fijas" /></div>
