...
export const fetchMatches = async (clubId?: string): Promise<Match[]> => {
    ...
    return matchesToReturn.map(match => ({
        ...
        durationMinutes: differenceInMinutes(new Date(match.endTime), new Date(match.startTime)),
    }));
};
...
export const addMatch = async (matchData: Omit<Match, 'id' | 'status' | 'confirmedPrivateSize' | 'organizerId' | 'privateShareCode'> & { creatorId?: string }): Promise<Match | { error: string }> => {
    ...
    const newMatch: Match = {
        id: (matchData as any).id || `match-${Date.now()}-${Math.random().toString(36).substring(7)}`, // Use provided ID if exists (for updates), else generate
        clubId: matchData.clubId,
        startTime: startTimeDate,
        endTime: endTimeDate,
        courtNumber: matchData.courtNumber,
        ...
        durationMinutes: differenceInMinutes(endTimeDate, startTimeDate),
    };
    ...
    if (originalMatch.status === 'confirmed' && updatedMatch.status === 'forming') {
        ...
        if (!hasOpenPlaceholder) {
            const newPlaceholder: Match = {
                ...
                durationMinutes: differenceInMinutes(new Date(updatedMatch.endTime), new Date(updatedMatch.startTime)),
  ...
  return userBookingsData.map(booking => {
    ...
    return {
      ...
      matchDetails: match ? {
        ...
        gratisSpotAvailable: match.gratisSpotAvailable,
